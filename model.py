...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    word = [i for i in word if i not in specials]
    for i in range(len(word)):
        dict[word[i]] = i + len(specials) 
    return dict
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    result = []
    for i in sen:
        if i not in token_to_id:
            result.append(token_to_id[unk_token])
        else:
            result.append(token_to_id[i])
    return result
...
def build_padding_mask(token_ids, pad_id):
    ...
    B, L = token_ids.shape
    pad_ids = torch.full((B, L), False)
    pad_ids = torch.where(token_ids == pad_id, pad_ids, True)
    return pad_ids[:, None, None, :]
...
def mask_attention_scores_with_neg_inf(scores, mask):
    ...
    scores = torch.where(mask == False, -torch.inf, scores)
    return scores
...
def softmax_attention_weights(masked_scores):
    ...
    masked_scores = torch.where(masked_scores != -torch.inf, torch.exp(masked_scores)/ torch.sum(torch.exp(masked_scores), axis = -1, keepdims = True),0)
...
def split_last_dim_into_heads(tensor, num_heads):
    ...
    tensor = tensor.reshape(B, L, num_heads, -1)
    return tensor
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    tmp = torch.transpose(multi_head_tensor, 1, 2)
...
def project_to_query_key_value(x, w_q, b_q, w_k, b_k, w_v, b_v):
    ...
    q = x @ (w_q.T) + b_q if b_q != None else x @ (w_q.T)
    k = x @ (w_k.T) + b_k if b_k != None else x @ (w_k.T)
    v = x @ (w_v.T) + b_v if b_v != None else x @ (w_v.T)
    return (q, k, v)
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    merge = merge_heads_back_to_model_dim(context)
